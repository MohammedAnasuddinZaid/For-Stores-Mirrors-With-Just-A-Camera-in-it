...
export default function SmartMirrorPage() {
  ...
  const handleCapture = useCallback(
    async (dataUrl: string) => {
      ...
      if (result) {
        setLandmarks(result);
        const img = engineRef.current.getCachedPersonImage();
        if (img) setPersonImage(img);
  ...
  if (step === 'idle') {
    return (
      <div
        className="min-h-screen bg-gradient-to-b from-gray-950 via-gray-900 to-gray-800 flex flex-col items-center justify-center text-white p-8 select-none"
        ...
        <div className="flex-1 relative">
          ...
          {vision && (
            <div className="absolute bottom-24 left-0 right-0 flex justify-center">
              <div
                className={`px-6 py-3 rounded-full text-lg font-medium backdrop-blur-md ${
  ...
  if (step === 'captured') {
    return (
      <div
        className="min-h-screen bg-gray-950 flex flex-col items-center justify-center p-4 text-white"
        onMouseMove={resetIdleTimer}
        onTouchStart={resetIdleTimer}
      >
        ...
        <div className="flex gap-4">
          <button
            onClick={() => setStep('camera')}
            className="px-8 py-4 bg-white/10 text-white rounded-2xl text-lg font-medium hover:bg-white/20 backdrop-blur-sm transition-all"
          >
            Retake
          </button>
          <button
            onClick={() => handleCapture(capturedImage!)}
            className="px-8 py-4 bg-brand-600 text-white rounded-2xl text-lg font-semibold hover:bg-brand-700 shadow-lg transition-all"
          >
            Try It On
          </button>
        </div>
  ...
  if (step === 'detecting') {
    return (
      <div
        className="min-h-screen bg-gray-950 flex flex-col items-center justify-center text-white p-8 select-none"
        onMouseMove={resetIdleTimer}
        onTouchStart={resetIdleTimer}
      >
        <div className="relative mb-8">
          <svg
            className="animate-spin w-16 h-16 text-brand-500"
            fill="none"
            viewBox="0 0 24 24"
          >
            <circle
  ...
  if (step === 'selecting') {
    return (
      <div
        className="min-h-screen bg-gray-950 flex flex-col p-4 text-white"
        onMouseMove={resetIdleTimer}
        onTouchStart={resetIdleTimer}
      >
        ...
        <button
          onClick={handleReset}
          className="mt-4 px-6 py-3 bg-white/10 text-white rounded-2xl text-lg font-medium hover:bg-white/20 backdrop-blur-sm transition-all"
        >
          Start Over
        </button>
      </div>
  ...
  if (step === 'result') {
    return (
      ...
      >
        ...
        {personImage && landmarks && selectedProduct && (
